...
do
    ...
    echo '{"type": "Feature","properties":{"id": ' $ids ', "nomin":' $nomin ', "tipovial": ' $tipovial ', "nomvial": ' $nomvial ', "nomasen": ' $nomasen ', "nomloc":  ' $nom_loc ', "nommun": ' $nom_mun ', "noment": ' $nom_ent ', "description": "<h1>Refugio</h1><hr><p><strong>Nombre: </strong>'$(echo $nomin  | sed 's/"//g')'</p><p><strong>Vialidad: </strong>'$(echo $nomvial  | sed 's/"//g')'</p><p><strong>Asentamiento: </strong>'$(echo $nomasen  | sed 's/"//g')'</p><p><strong>Localidad: </strong>'$(echo $nom_loc  | sed 's/"//g')'</p><p><strong>Tipo inmueble: </strong>'$(echo $tipovial  | sed 's/"//g')'</p>"}, "geometry": { "type": "Point","coordinates": [' $lon ',' $lat ']}},' >> ../html/body.html
    ids=$((ids + 1))
done
## Remove last comma
